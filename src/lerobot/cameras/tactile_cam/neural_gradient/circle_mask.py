...
def infer_gradient(feature):
    model = torch.load('model/model2.pt', map_location=torch.device('cpu'), weights_only=False)
    ...
    for file_pointer in range(len(ballfiles)):
        ...
        if exists(crop_filepath):
            ...
            with open(crop_filepath, 'r+') as f:
                ...
                key, _, new_center, new_radius = get_touch_mask_by_selection(pre, img_bg, img, ball_radius_p, (x, y), radius, visual=True, dir=data_folder)
            ...
            with open(crop_filepath, "w") as f:
                key, _, center, radius = get_touch_mask_by_selection(pre, img_bg, img, ball_radius_p, predicted=False, visual=True, dir=data_folder)
