...
class QuickROICalibrator:
    def __init__(self, output_size=None, save_dir=None):
        ...
        初始化
        Args:
            output_size: 输出图像固定尺寸 (width, height)，None表示自动根据ROI计算
            save_dir: 标定参数保存目录
        """
        self.output_size = output_size
        self.fixed_output_size = output_size is not None  # 是否使用固定尺寸
    ...
    def _compute_homography(self):
        ...
        # 如果没有指定固定输出尺寸，则使用ROI的实际尺寸
        if self.output_size is None:
            self.output_size = (avg_width, avg_height)
            print(f"[INFO] 根据ROI自动计算输出尺寸: {avg_width}x{avg_height}")
        else:
            print(f"[INFO] 使用固定输出尺寸: {self.output_size[0]}x{self.output_size[1]}")
            print(f"[INFO] ROI原始像素尺寸约为: {avg_width}x{avg_height}")
        ...
        while True:
            try:
                ...
                if key == ord('r'):
                    ...
                    if not self.fixed_output_size:
                        self.output_size = None  # 重置自动计算的尺寸
...
def main():
    ...
    camera_config = TactileCameraConfig(
        ...
        exposure=600,        # 曝光值
        wb_temperature=4000, # 白平衡色温
        r_gain=1.05,          # RGB增益
        g_gain=1.0,
        b_gain=1.0
    )
    
    calibrator = QuickROICalibrator(output_size=None)  # None = 自动根据ROI尺寸计算
